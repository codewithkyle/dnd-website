...
class DeleteMonsterButton extends HTMLElement {
	...
	private delete: EventListener = async () => {
		...
		if (request.ok) {
			...
			console.error(error);
			snackbar({
				message: "Failed to delete monster, try again later.",
				closeable: true,
				force: true,
				duration: 3,
			});
...
customElements.define("delete-monster-button", DeleteMonsterButton);
