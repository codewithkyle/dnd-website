...
class MonsterSheet extends HTMLElement {
	...
	private async save() {
		...
		if (request.ok) {
			...
			if (!response.success) {
				...
				snackbar({
					message: "Monster successfully saved.",
					closeable: true,
					force: true,
					duration: 3,
				});
			...
			console.error(error);
			snackbar({
				message: "Failed to save monster, try again later.",
				closeable: true,
				force: true,
				duration: 3,
			});
...
customElements.define("monster-sheet", MonsterSheet);
